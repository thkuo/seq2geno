...
cd $SEQ2GENO_HOME
trap 'echo "Errors in setting conda channels"; exit ' ERR 
...
#cd $SEQ2GENO_HOME
#trap 'echo "Errors in downloading the core environment"; exit ' ERR EXIT 
#
...
cd $SEQ2GENO_HOME
trap 'echo "Errors in setting up the core environment"; exit ' ERR 
...
conda deactivate
cd $SEQ2GENO_HOME
trap 'echo "Errors in installation of Roary dependecies"; exit ' ERR EXIT 
...
trap 'echo "Errors in installation of the process-specific environments failed"; exit ' ERR 
{
	conda deactivate
} || {
	source deactivate
